...
const single_transferable_vote_all_profiles = function (profile) {
...
const nxy = function (profile, x, y) {
  ...
  for (ballot of profile) {
    for (preference of ballot.preference) {
      if ((Array.isArray(preference) && preference.includes(x)) || preference == x) {
      ...
      if ((Array.isArray(preference) && preference.includes(y)) || preference == y) {
        break;
      }
    }
  }
...
// In the given profile, bumps the alternative to the top of the list for up to n
// ballots that do not already have the alternative at the top
// returns the number of ballots that were thus modified (may be less than n if all
// ballots already rank alternative at the the top)
// NOTE: Can technically return results > n (WIP)
const bump_alternative = function (profile, alternative, n) {
  let bumps = 0;
  for (ballot of profile) {
    if (bumps < n) {
      if (
        // singleton case
        (!Array.isArray(ballot.preference[0]) && ballot.preference[0] != alternative) ||
        // array case
        (!ballot.preference[0].includes(alternative))
      ) {
        const index_alternative = ballot.preference.find(element => element == alternative || (Array.isArray(element) && element.includes(alternative)));
...
const find_coalition = function (profile) {
  ...
  // go through all the voting iterations (from back to front, skipping the first)
  const vote_iterations = single_transferable_vote_all_profiles(profile).reverse();
  ...
  for (const profile_n of vote_iterations.slice(1)) {
    ...
    const viable_alternatives = alternatives_n.filter(function (element) { return !winner.includes(element) && !checked_alternatives.includes(element) });
    for (alternative of viable_alternatives) {
      ...
      for (target of winner) {
        ...
        // now with this agent pool, make alternative the top choice for an incremental number of agents
        for (let n = 0; n < cardinality_profile(agent_pool);) {
          const actual_n = bump_alternative(agent_pool, alternative, n);

          // now plug this coalition in with the rest of the agents and see if we win
          ...
          // optimization: we need at least a number of agents that is equal to at least half the plurality score difference
...
// naive algorithm
const naive_coalition_finder = function (profile) {
  const winner = single_transferable_vote(profile);
  const alternatives = alternatives_in_profile(profile);
  const viable_alternatives = alternatives.filter(function (element) { return !winner.includes(element) });
  // go through all the voting iterations (from back to front, skipping the first)
  for (alternative of viable_alternatives) {
    // get the subsets (pools of agents) nxy where x beats y
    for (target of winner) {
      ...
      const actual_n = bump_alternative(agent_pool, alternative, cardinality_profile(agent_pool));
      // now plug this coalition in with the rest of the agents and see if we win
      const prime = [...agent_pool, ...agent_pool_complement];
      console.log("Intermediate outcome");
      const winner_prime = single_transferable_vote(prime);
